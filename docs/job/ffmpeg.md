...
*ffmpeg_parse_options()* 这个部分会解析命令行的参数，打开输入输出文件，根据参数配置```demux,mux,decode,encode,filter```等任务，这里就会调用到相关的代码的入口（```ifile_open,of_open,dec_create,fg_create```），其中```encode```和```mux```都在```of_open```调用，```enc```是在```of_open```的```create_stream```调用```ost_add```

*transcode()*个部分会启动并且执行任务
